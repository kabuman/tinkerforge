...
import de.kabuman.tinkerforge.legoschiff.services.LegoSchiffService;
import de.kabuman.tinkerforge.legoschiff.services.LegoSchiffServiceImpl;
...
/**
 * LegoSchiff Application
 * 
 * Contains the complete logic to operate the Legoschiff
 */
public class LegoSchiffAppl {
	 ...
	 */
	public Exception legoSchiffLauncher(boolean restart){
		...
		try {
			...
			System.out.println("legoSchiffLauncher:: IOException");
			return e1;
		} catch (TimeoutException e) {
			System.out.println("legoSchiffLauncher:: TimeoutException");
			return e;
		}
		
		System.out.println("legoSchiffLauncher:: Start");
		...
		try {
			...
			System.out.println("legoSchiffLauncher:: writeStartMsgToConsole(), refreshDisplay(): TimeoutException");
			return e;
		}
		
		// Install Listener to ensure motor stop if joystick pos is nearly zero
		new EnsureMotorStop((MotorAbstractService) legoService.getMotorAndRudderService(), rcService.getJoystick());
...
//		ConfigServiceImpl.getInstance().getConnect().joinThread();
	...
	private void writeStartMsgToConsole() throws TimeoutException{
		System.out.println("Anwendung \"Legoschiff\" erfolgreich gestartet\n");
...
//		deactivateAllCallbackListener();
		...
		legoService.getStackService().writeStatusToConsole();
...
//		activateAllCallbackListener();
	...
	private void configServices() throws IOException, TimeoutException {
		...
		legoService = new LegoSchiffServiceImpl();
	}
...
//                System.out.println("Value: " + Integer.toBinaryString(valueMask));
	            ...
	            if (isInterruptedBy(TASTER2_RESET,interruptMask )){
	            	if (!isSwitchedON(TASTER2_RESET,valueMask )){
	                    ...
	                    legoService.getMotorAndRudderService().calculateAndSet(SHORT_ZERO,SHORT_ZERO);
...
//				System.out.println("x/y="+x+"/"+y);
				...
				legoService.getMotorAndRudderService().calculateAndSet(xTenClustered,yTenClustered);
			}
		});
	}
	...
	private void controlMaxVelocityLimiter(){
		...
		legoService.getVehicleService().setMaxVelocity(getMaxVelocityFromRotaryPoti());
		legoService.getMotorAndRudderService().setVelocityMax(legoService.getVehicleService().getMaxVelocity());
...
//					System.out.println("=> Erkannt: Limit-Regler flimmert. Unterdrückt.");
					...
					// do the job
					legoService.getVehicleService().setMaxVelocity(TransformerService.rotaryDegreeToVelocity(position));
					legoService.getMotorAndRudderService().setVelocityMax(legoService.getVehicleService().getMaxVelocity());
					rcService.refreshMaxVelocity(legoService.getVehicleService());
				}
...
//		activateAllCallbackListener();
	...
	private void executeTest(int position, int velocity, long duration, boolean stopAfterTest){
		StopWatchApplService.getInstance().start();
		legoService.getMotorAndRudderService().calculateAndSet(position,velocity);
	...
	private void motorAndRudderTest(){
		...
		legoService.getVehicleService().setMaxVelocity(TransformerService.rotaryDegreeToVelocity((short)0));
		legoService.getMotorAndRudderService().setVelocityMax(legoService.getVehicleService().getMaxVelocity());
	...
	private short getMaxVelocityFromRotaryPoti(){
		...
		try {
			...
			legoService.getVehicleService().setMaxVelocity(maxVelocity);
