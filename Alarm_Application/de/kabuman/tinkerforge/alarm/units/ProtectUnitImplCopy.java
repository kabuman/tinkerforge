...
public class ProtectUnitImplCopy extends AbstractUnit implements ProtectUnit{
	IPConnection ipcon;
	 ...
	 * @throws IOException
	 */
	public ProtectUnitImplCopy(CfgProtectUnit cfgProtectUnit){
	...
	private void connectSensors(){
		...
		if (cfgProtectUnit.getAi() > 0){
			try {
				...
				waterSensorItem = new WaterSensorItemImpl(
						...
						false);
				replacePreviousInstance(waterSensorItem);
			} catch (TimeoutException e) {
				LogControllerImpl.getInstance().createTechnicalLogMessage(cfgProtectUnit.getUnitName(), "Connect", "Water Sensor failed. Timeout");
			}
		}
		...
		if (cfgProtectUnit.getTp() > 0){
			try {
				...
				replacePreviousInstance(temperatureSensorItem);
		...
		if (cfgProtectUnit.getHm() > 0){
			try {
				...
				replacePreviousInstance(humiditySensorItem);
